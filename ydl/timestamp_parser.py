...
def parse_chapters(
    text: str, video_duration: int | float | str | None = None, return_int: bool = True
):
    """
    ...
    adjust_chapter_boundaries(chapters, video_duration)  # type:ignore
...
def adjust_chapter_boundaries(
    chapters: list[dict], video_duration: float | int | None = None
):
    """
...
def check_start_and_end(chapter_list: list[dict[str, str | int]]):
    for idx, chapter in enumerate(chapter_list):
        if chapter.get("end") and chapter.get("start") > chapter.get("end"):  # type:ignore
